...
module.exports = function() {
    this.type('object', {type: 'base'}, function() {
        ...
        this.method('keyOf', function(sourcePath) {
            ...
            return function(path, key, dataWrapper) {
                if (dataWrapper.getKeys(sourcePath).indexOf(key) < 0) {
                    scope.error(path, "Reference to unexisting key " + key + " at path " + sourcePath);
                }
            };
        });
        ...
        this.method('memberOf', function(sourcePath) {
            var scope = this.scope;
            return function(path, key, dataWrapper) {
                var object = dataWrapper.get(sourcePath);
                if (dataWrapper.get(sourcePath).indexOf(key) < 0) {
                    scope.error(path, "Reference to unexisting value " + key + " at path " + sourcePath);
                }
            };
        });
