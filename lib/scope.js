...
Scope.prototype.getType = function(type) {
    ...
    if (!ctor) {
        throw new SchemaError("Validator type %s not found", type);
    }
    return ctor;
};
...
Scope.prototype.addType = function(type, ctor) {
    if (this.hasType(type)) {
        throw new SchemaError("Validator type %s already defined in current scope", type);
    } else {
...
Scope.prototype.addProperty = function(prop) {
    if (this.hasProperty(prop)) {
        throw new SchemaError("Scope property " + prop + " already defined in current scope");
    }
...
Scope.prototype.getValidator = function(props, schema) {
    if (!props.type) {
        throw new SchemaError("Property type missing in Validator parameters");
    }
    ...
    _.forOwn(props, function(value, name, obj) {
        if (!this.hasProperty(name) && !type.hasProperty(name)) {
            throw new SchemaError("Validator property " + name + " is unknown in current scope");
        }
    }, this);
